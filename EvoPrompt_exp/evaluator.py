...
class Evaluator(object):
    def __init__(self, args) -> None:
        ...
        if args.position in ["icl", "pre"]:
            ...
            self.template = None
        # print(self.template)
        self.model_name = args.language_model.split("/")[-1] # 就是儲存模型名稱
...
class CLSEvaluator(Evaluator):
    def __init__(self, args):
        super(CLSEvaluator, self).__init__(args) # 調用父類別 Evaluator 建構子
        self.verbalizers = get_dataset_verbalizers(args.dataset) # 儲存該任務的類別標籤
        if "gpt" not in args.language_model:
            self.verbalizer_ids = [
                self.tokenizer.convert_tokens_to_ids(v) for v in self.verbalizers
            ]
        # FIXME 目前改了儲存測試及驗證資料集的path
        args.dev_file = (
            f"./data/cls/{args.dataset}/seed5/dev_{args.sample_num}.txt"
            if args.dev_file is None
            else args.dev_file
        )
        args.test_file = (
            f"./data/cls/{args.dataset}/seed5/test.txt"
