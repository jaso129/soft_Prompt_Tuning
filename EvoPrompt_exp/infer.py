...
def evaluate_single_prompt(evaluator, args, prompt_pre, output=None):
    if args.task == 'cls':
        eval_src, eval_tgt = load_cls_data( # 傳入任務類別的標籤以及任務測試集
...
if __name__ == "__main__":
    # TODO 執行順序1
    eval_args = parse_args() # 將解析後的參數儲存在 eval_args中
    print(eval_args)

    task2evaluator = { # 將任務類型與其評估器對應
        "cls": CLSEvaluator,
        "sum": SumEvaluator,
        "sim": SimEvaluator,
    }
    # ICL
    scores = []
    evaluator = task2evaluator[eval_args.task](eval_args) # 創建提示評估器對象
